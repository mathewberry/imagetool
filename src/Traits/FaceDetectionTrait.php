...
trait FaceDetectionTrait
{
    ...
    public function detectFaces($detection_data = 'detection.dat')
    {
        ...
        $this->detection_data = unserialize(file_get_contents(__DIR__ . '/../' . $detection_data));

        $width = imagesx($this->canvas);
        $height = imagesy($this->canvas);

        $diff_width = 320 - $width;
        $diff_height = 240 - $height;
        ...
        if($ratio != 0 ) {
            $this->reduced_canvas = imagecreatetruecolor($width / $ratio, $height / $ratio);

            imagecopyresampled($this->reduced_canvas, $this->canvas, 0, 0,0, 0, ($width / $ratio),
                ($height / $ratio), $width, $height);

            $stats = $this->imageStats($this->reduced_canvas);
            ...
            $stats = $this->imageStats($this->canvas);
            $this->face = $this->detectGreedyBigToSmall($stats['ii'], $stats['ii2'], $stats['width'], $stats['height']);
        }
